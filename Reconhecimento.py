...
class ReconhecimentoFacial:
    ...
    def selecionar_landmarks_principais(self, landmarks):
    ...
    def calcular_distancia_normalizada(self, landmarks):
        distancias = []
    ...
    def calcular_diferenca_landmarks(self, landmarks1, landmarks2):
        distancias1 = self.calcular_distancia_normalizada(landmarks1)
        distancias2 = self.calcular_distancia_normalizada(landmarks2)
            ...
            while True:
                ...
                if results.multi_face_landmarks:
                    for face_landmarks in results.multi_face_landmarks:
                        landmarks_principais = self.selecionar_landmarks_principais(face_landmarks.landmark)
    ...
    def ajustar_tolerancia_por_distancia(self, landmarks_atual):
        distancias = self.calcular_distancia_normalizada(landmarks_atual)
    ...
    def verificar_rosto_autorizado(self):
        try:
        ...
        with mp_face_mesh.FaceMesh(static_image_mode=False, max_num_faces=1, min_detection_confidence=0.5) as face_mesh:
            while True:
                ...
                if results.multi_face_landmarks:
                    for face_landmarks in results.multi_face_landmarks:
                        landmarks_atual = self.selecionar_landmarks_principais(face_landmarks.landmark)
                        limite_diferenca = self.ajustar_tolerancia_por_distancia(landmarks_atual)
                        diferenca = self.calcular_diferenca_landmarks(landmarks_atual, rosto_autorizado)
...
if __name__ == '__main__':
    ...
    def menu():
        while True:
            ...
            if opcao == '1':
                f.capturar_rosto_autorizado()
            elif opcao == '2':
                if f.verificar_rosto_autorizado():
                    print("Dispositivo desbloqueado.")
                else:
